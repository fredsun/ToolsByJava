...
public class ExcelProcess {
    ...
    //构造函数初始化
    public ExcelProcess(String Path, String SheetName) throws Exception{
...
//    public static Object[][] getTestDataOrigin(String execlFilePath,String sheetName) throws IOException {
    ...
    public static Object[][] getTestData(String execlFilePath,String sheetName) throws IOException {
        ...
        for (int i=1;i<rowCount + 1;i++){
            ...
            if (row.getCell(4) != null){
                System.out.println(row.getCell(4)+" cellType "+row.getCell(4).getCellType());
                Cell cell1 = row.getCell(1);
                if (cell1 == null ){
                    cell1 = row.createCell(1);
                }
                cell1.setCellValue(row.getCell(4).getStringCellValue());
//                setCellData(i, 1, row.getCell(4));
            }else{
...
//            }
    ...
    public void transDataTo(ExcelProcess fromProcess, ExcelProcess toProcess){
        XSSFSheet fromSheet = fromProcess.ExcelWSheet;
        int fromRowCount = fromSheet.getLastRowNum() - fromSheet.getFirstRowNum();
        XSSFSheet toSheet = toProcess.ExcelWSheet;
        int toRowCount = toSheet.getLastRowNum() - toSheet.getFirstRowNum();
        for (int i = 1; i < fromRowCount; i++){
            Row fromRow = fromSheet.getRow(i);
            String fromCellName = fromRow.getCell(3).getStringCellValue();
            Cell fromCellRemainTimes = fromRow.getCell(6);
            Cell fromCellDeathDate = fromRow.getCell(7);

            for (int j = 1; j < toRowCount; j++){
                Row toRow = toSheet.getRow(j);
                String toCellName = toRow.getCell(0).getStringCellValue();
                Cell toCellAllTimes = toRow.getCell(6);
                Cell toCellDeathDate = toRow.getCell(5);

                if (fromCellName.equals(toCellName)){
                    if (toCellDeathDate == null){
                        toCellDeathDate = toRow.createCell(5);
                    }
                    if (fromCellDeathDate.getCellType().equals(CellType.NUMERIC)){
                            toCellDeathDate.setCellType(CellType.NUMERIC);
                            SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
                            Calendar calendar = new GregorianCalendar(1900,0,-1);
                            Date d = calendar.getTime();
                            Date dd = DateUtils.addDays(d,Integer.valueOf((int) fromCellDeathDate.getNumericCellValue()));
                            toCellDeathDate.setCellValue(sdf.format(dd));
                    }else {

                    }
                    System.out.println("fromCellDeathDate"+fromCellDeathDate.getNumericCellValue());
                }
            }
        ...
        try {
            fileOut = new FileOutputStream(toProcess.filePath);
            // 将内容写入excel 文件中
            toProcess.ExcelWBook.write(fileOut);
            // 调用flush方法强制刷新写入文件
            fileOut.flush();
            // 关闭文件输出流对象
            fileOut.close();
